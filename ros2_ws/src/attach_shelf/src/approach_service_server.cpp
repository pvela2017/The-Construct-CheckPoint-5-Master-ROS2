...
public:
    ApproachServiceServer() : Node("approach_service_server_node")
    {
        ...
        // Service callback
        service_ = this->create_service<attach_shelf::srv::GoToLoading>( "/approach_shelf",
...
private:
    ...
    std::string source_frame_ = "robot_front_laser_base_link";
    bool result_ = true;
    bool cart_frame_flag_ = true;
    bool to_position_flag_ = false;
    bool in_position_flag_ = false;

    void approachCallBack(const std::shared_ptr<attach_shelf::srv::GoToLoading::Request> request,
    ...
    void pubTF() 
    {
        ...
        {
            if (this->laser_msg_->intensities[i] >= 8000 && i >= izq_index && i <= der_index) 
            {
    ...
    void cart_approach()
    {
      try
      {
        geometry_msgs::msg::TransformStamped transformStamped =
            tf_->lookupTransform(source_frame_, target_frame_, tf2::TimePointZero, tf2::durationFromSec(0.1));
